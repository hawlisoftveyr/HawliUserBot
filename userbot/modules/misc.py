...
async def randomise(items):
    ...
    if len(itemo) < 2:
        await items.edit(
            "`2 veya daha fazla eşya gerekli. Daha fazla bilgi için .hawli random komutunu gir.`"
        )
        return
...
async def bot_support(wannahelp):
    ...
    await wannahelp.edit("[Buradan](http://t.me/HawliSupport) destek grubumuza ulaşabilirsiniz.")


@register(outgoing=True, pattern="^.creator$")
async def creator(e):
    await e.edit("Bu bot \n"
                 "[HawliJojuk](https://t.me/hawlijojuk) tarafından geliştirilmiştir. \n")


@register(outgoing=True, pattern="^.readme$")
async def reedme(e):
    await e.edit("[Asena README.md](https://github.com/hawlisoftveyr/HawliUserBot/blob/master/README.md)")
...
async def repo_is_here(wannasee):
    ...
    await wannasee.edit("[Hawli Repo](https://github.com/hawlisoftveyr/hawliuserbot)")
...
CMD_HELP.update({
    'sleep':
    '.sleep <saniye>\
\nKullanım: Hawli de bir insan, o da yoruluyor. Ara sıra biraz uyumasına izin ver.'
})
...
CMD_HELP.update({
    "readme":
    ".readme\
\nKullanım: Hawli botunun GitHub'daki README.md dosyasına giden bir bağlantı."
})
