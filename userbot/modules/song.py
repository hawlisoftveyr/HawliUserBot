...
async def deezl(event):
    ...
    async with bot.conversation(chat) as conv:
        ...
        if sarkilar.audio:
            await event.client.send_read_acknowledge(conv.chat_id)
            await event.client.send_message(event.chat_id, f"@HawliUserBot ile yüklendi.", file=sarkilar.message)
            await event.delete()
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            await event.client.send_message(event.chat_id, f"`{sarkilar.buttons[0][0].text}` | @HawliUserBot ile yüklendi.", file=sarki.message)
            await event.delete()
