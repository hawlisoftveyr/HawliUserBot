...
async def ekle(event):
    ...
    if event.is_private:
        await event.edit("`Ekle komutu kullanıcıları sohbete ekler, özel mesaja değil!`")
    else:
...
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            "`Ban Hatası! Hawli Yetkilisini yasaklayamam.`"
        )
        return
...
async def spider(spdr):
    ...
    if user.id in BRAIN_CHECKER:
        await spdr.edit(
            "`Mute Hatası! Hawli Yetkilisini susturamam.`"
        )
        return
...
async def gspider(gspdr):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit("`Gmute Hatası! Hawli Yetkilisini küresel olarak susturamam.`")
        return
...
async def kick(usr):
    ...
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            "`Kick Hatası! Hawli Yetkilisini gruptan tekmeleyemem.`"
        )
        return
...
async def warn(event):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await event.edit("`Warn Hatası! Hawli Yetkilisini uyaramam.`")
        return
