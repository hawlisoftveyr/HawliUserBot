...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/hawlisoftveyr/HawliUserBot.git")

# Ayrıntılı konsol günlügü
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///hawli.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@HawliUserBot | ")
...
with bot:
    try:
        bot(JoinChannelRequest("@HawliUserBot"))
        bot(JoinChannelRequest("HawliSupport"))
        ...
        async def handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @HawliUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Hawli açabilirsin; Kanala bak` @HawliUserBot')
            else:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@HawliUserBot":
                rev_text = query[::-1]
                buttons = paginate_help(0, moduller, "helpme")
                result = builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text="{}\nYüklenen Modül Sayısı: {}".format(
                        "Merhaba! Ben @HawliUserBot kullanıyorum!\n\nhttps://github.com/hawlisoftveyr/HawliUserBot", len(moduller)),
                    buttons=buttons,
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @HawliUserBot",
                    text=f"@HawliUserBot ile güçlendirildi",
                    buttons=[],
                    link_preview=True
                )
                ...
                result = builder.article(
                    "© @HawliUserBot",
                    text="""@HawliUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/HawliUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/HawliSupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/hawlisoftveyr/HawliUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                reply_pop_up_alert = "Lütfen kendine bir @HawliUserBot aç, benim mesajlarımı düzenlemeye çalışma!"
            ...
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                if len(cmdhel) > 90:
                    help_string = str(CMD_HELP[modul_name])[
                        :90] + "\n\nDevamı için .hawli " + modul_name + " yazın."
                else:
                    help_string = str(CMD_HELP[modul_name])
